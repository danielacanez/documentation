...
.. example::
   ...
   or :abbr:`CC (Carbon Copy)` email addresses (e.g. commercial@gmail.com, commercial@odoo.net),
...
:guilabel:`Save`, and the system parameter takes immediate effect.
...
detection`. If enabled, Odoo will only require the Local-part to match for routing an incoming
email. If this feature is disabled, Odoo will require the whole email address to match for routing
an incoming email.
